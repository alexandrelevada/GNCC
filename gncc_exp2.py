...
#%%%%%%%%%%%%%%%%%%%%  Data loading
# GNCC x KNN x SVM x XGBoost
#X = skdata.fetch_openml(name='UMIST_Faces_Cropped', version=1)    
#X = skdata.fetch_openml(name='Olivetti_Faces', version=1)           # 0.43 to 0.63
...
#X = skdata.fetch_openml(name='CIFAR_10_small', version=1)          # 0.43 to 0.63
...
#X = skdata.fetch_openml(name='one-hundred-plants-margin', version=1)
#X = skdata.fetch_openml(name='LED-display-domain-7digit', version=1)
#X = skdata.fetch_openml(name='yeast', version=1)                    # 0.2 to 0.5
#X = skdata.fetch_openml(name='Satellite', version=1)
#X = skdata.fetch_openml(name='SPECT', version=1)
...
#X = skdata.fetch_openml(name='garber-2001', version=1)
#X = skdata.fetch_openml(name='SRBCT', version=1)
#X = skdata.fetch_openml(name='colon-cancer', version=1)
...
for size in sample_list:
    ...
    # Apply k-NN
    knn = KNeighborsClassifier(n_neighbors=5)        # estava 7
...
plt.plot(sample_list, acc_knn, color='red', label='KNN')
plt.legend()
